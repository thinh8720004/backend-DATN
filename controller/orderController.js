...
const getAllOrders = async (req, res) => {
  ...
  // Điều chỉnh end_date để bao gồm cả ngày cuối
  ...
  if (customerName) {
    const isNumber = !isNaN(Number(customerName)); // Kiểm tra nếu customerName là số
    queryObject.$or = [
      { "user_info.name": { $regex: `${customerName}`, $options: "i" } }, // Tìm theo tên
      ...(isNumber ? [{ invoice: Number(customerName) }] : []), // Tìm theo hóa đơn nếu là số
    ];
  }
  ...
  if (startDate && endDate) {
    queryObject.updatedAt = {
      $gte: start_date, // Bao gồm start_date
      $lte: end_date,   // Bao gồm end_date
    };
  }
...
const updateOrder = async (req, res) => {
  ...
  try {
    ...
    // Xác định giá trị paymentStatus mới dựa trên newStatus
    let newPaymentStatus = order.paymentStatus; // Giữ nguyên mặc định

    if (newStatus === "Delivered") {
      newPaymentStatus = true; // Giao hàng -> paymentStatus = true
    } else if (["Pending", "Processing", "Cancel"].includes(newStatus)) {
      newPaymentStatus = false; // Các trạng thái này -> paymentStatus = false
    }
